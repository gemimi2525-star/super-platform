...
UA="Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) verify-parity/1.0"

# ── 1. Fetch build-info ───────────────────────────────────────
echo -e "${BOLD}▸ Fetching /api/build-info${NC}"
BI_RESPONSE=$(curl -s --connect-timeout 10 --max-time 30 -H "User-Agent: $UA" "$BUILD_INFO_URL" 2>/dev/null)

if [ -z "$BI_RESPONSE" ]; then
    fail "Cannot reach $BUILD_INFO_URL (empty response)"
    echo ""
    echo "RESULT: FAIL ($FAILURES failures)"
    exit 1
fi

# Check if response is JSON (not Vercel security page)
if ! echo "$BI_RESPONSE" | jq . >/dev/null 2>&1; then
    fail "Response is not JSON — possible Vercel bot protection"
    info "Try running from a whitelisted IP or in GitHub Actions CI"
    echo ""
    echo "RESULT: FAIL ($FAILURES failures)"
    exit 1
fi

echo "$BI_RESPONSE" | jq . 2>/dev/null
echo ""
...
INT_RESPONSE=$(curl -s --connect-timeout 10 --max-time 30 -H "User-Agent: $UA" "$INTEGRITY_URL" 2>/dev/null)

if [ -z "$INT_RESPONSE" ]; then
    fail "Cannot reach $INTEGRITY_URL (empty response)"
    echo ""
    echo "RESULT: FAIL ($FAILURES failures)"
    exit 1
fi

if ! echo "$INT_RESPONSE" | jq . >/dev/null 2>&1; then
    fail "Response is not JSON — possible Vercel bot protection"
    info "Try running from a whitelisted IP or in GitHub Actions CI"
    echo ""
    echo "RESULT: FAIL ($FAILURES failures)"
...
echo "$INT_RESPONSE" | jq . 2>/dev/null
echo ""
