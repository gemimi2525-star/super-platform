...
exports.CoreOSStateStore = exports.INITIAL_STATE = exports.INITIAL_SECURITY_CONTEXT = void 0;
...
// INITIAL STATE
// ═══════════════════════════════════════════════════════════════════════════
exports.INITIAL_SECURITY_CONTEXT = {
    authenticated: false,
    userId: null,
    role: 'guest',
    stepUpActive: false,
...
exports.INITIAL_STATE = {
    ...
    activeSpaceId: index_js_1.DEFAULT_SPACE_ID, // Phase L
    processes: {},
    activeCapabilities: [],
    contextStack: ['core.finder'],
    cognitiveMode: 'calm',
    security: exports.INITIAL_SECURITY_CONTEXT,
    pendingStepUp: null,
};
// ═══════════════════════════════════════════════════════════════════════════
    ...
    switch (action.type) {
        ...
        case 'WINDOW_MINIMIZE': {
            ...
            if (!window)
                return state;
            const newFocused = state.focusedWindowId === action.windowId
                ? state.windowOrder.filter(id => id !== action.windowId && state.windows[id]?.state === 'active').pop() ?? null
                : state.focusedWindowId;
            return {
                ...state,
                windows: {
        ...
        case 'WINDOW_MINIMIZE_ALL': {
            const minimizedWindows = Object.fromEntries(Object.entries(state.windows).map(([id, w]) => [
                id,
                { ...w, state: 'minimized' },
            ]));
            return {
        ...
        case 'CAPABILITY_DEACTIVATE': {
            return {
                ...state,
                activeCapabilities: state.activeCapabilities.filter(c => c !== action.capabilityId),
            };
        }
        ...
        case 'WINDOW_MOVE_TO_SPACE': {
            ...
            const newFocusedWindowId = state.focusedWindowId === action.windowId && action.spaceId !== state.activeSpaceId
                ? null
                : state.focusedWindowId;
            return {
                ...state,
...
// STATE STORE
// ═══════════════════════════════════════════════════════════════════════════
class CoreOSStateStore {
    state;
    listeners = new Set();
    constructor(initial = exports.INITIAL_STATE) {
        this.state = initial;
    }
    /**
     ...
     */
    reset() {
        this.state = exports.INITIAL_STATE;
        this.notifyListeners();
    }
}
exports.CoreOSStateStore = CoreOSStateStore;
// ═══════════════════════════════════════════════════════════════════════════
// SINGLETON
// ═══════════════════════════════════════════════════════════════════════════
let instance = null;
