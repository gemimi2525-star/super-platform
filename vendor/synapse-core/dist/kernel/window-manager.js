...
exports.CoreOSWindowManager = void 0;
...
class CoreOSWindowManager {
     ...
     */
    openWindow(capabilityId, correlationId, contextId, spaceId // Phase O: explicit space targeting
    ) {
     ...
     * Close a window
     */
    closeWindow(windowId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const eventBus = (0, event_bus_js_1.getEventBus)();
     ...
     * Focus a window
     */
    focusWindow(windowId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const eventBus = (0, event_bus_js_1.getEventBus)();
     ...
     * Minimize a window
     */
    minimizeWindow(windowId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const eventBus = (0, event_bus_js_1.getEventBus)();
     ...
     */
    restoreWindow(windowId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const eventBus = (0, event_bus_js_1.getEventBus)();
     ...
     */
    getWindowsForCapability(capabilityId) {
        const state = (0, state_js_1.getStateStore)().getState();
     ...
     */
    getPrimaryWindowIdForCapability(capabilityId) {
        const state = (0, state_js_1.getStateStore)().getState();
        ...
        return windows.reduce((highest, w) => w.zIndex > highest.zIndex ? w : highest).id;
    }
    /**
    ...
    getDiscoverableCapabilities() {
        ...
        return allCapabilities.filter((capabilityId) => {
     ...
     */
    getWindowLifecycleState(windowId) {
        const state = (0, state_js_1.getStateStore)().getState();
        const window = state.windows[windowId];
        if (!window) {
            return null;
     ...
     */
    getPersistedWindowsInActiveSpace() {
        const state = (0, state_js_1.getStateStore)().getState();
        return Object.values(state.windows)
            .filter(w => w.spaceId === state.activeSpaceId &&
            w.state === 'minimized');
     ...
     */
    restoreWindowById(windowId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const eventBus = (0, event_bus_js_1.getEventBus)();
        const state = store.getState();
        const window = state.windows[windowId];
     ...
     */
    restoreAllInActiveSpace(correlationId) {
     ...
     */
    getFocusableWindowIdByIndex(index) {
        const focusableIds = this.getFocusableWindowIds();
    ...
    getLastMinimizedWindowId() {
        ...
        return minimizedWindows.reduce((latest, w) => w.zIndex > latest.zIndex ? w : latest).id;
    }
    /**
     * Minimize the focused window (must be in ACTIVE SPACE)
     * Returns the windowId that was minimized, or null
     */
    minimizeFocusedWindow(correlationId) {
     ...
     */
    closeFocusedWindow(correlationId) {
        const focusedWindowId = this.getFocusedWindowId();
        if (!focusedWindowId) {
            return null;
        }
     ...
     * Phase N: Now space-scoped
     */
    restoreLastMinimizedWindow(correlationId) {
     ...
     */
    minimizeAllInActiveSpace(correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const state = store.getState();
     ...
     */
    moveWindowToSpace(windowId, spaceId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        const state = store.getState();
        const window = state.windows[windowId];
        if (!window) {
     ...
     */
    switchSpace(spaceId, correlationId) {
        const store = (0, state_js_1.getStateStore)();
        store.dispatch({
            type: 'SPACE_SWITCH',
            spaceId,
    ...
    getSpacesWithWindows() {
        ...
        spaces.add(index_js_1.DEFAULT_SPACE_ID);
        return Array.from(spaces);
    }
}
exports.CoreOSWindowManager = CoreOSWindowManager;
// ═══════════════════════════════════════════════════════════════════════════
// SINGLETON
// ═══════════════════════════════════════════════════════════════════════════
let instance = null;
